public class CircuitBoard {
    ...
    */
    ...
    public static void  delete(String newDir,int finalX,int finalY,int startX,int startY){
        int x = startX;
        int y = startY;
   ...
   public static void change(String newDir,int finalX,int finalY,int startX,int startY,double resistance){
       ...
       int x = startX;
        ...
        while(x != finalX && y != finalY){
            ...
            int a = points[x][y].dirToDirLoc(newDir);
            if (first){
                points[x][y].wires[a][1] = Double.toString(resistance);
            }
            else{
                points[x][y].wires[a][1] = "wire";
            }
            first = false;
            //move in nextDir direction
